...
let fileUploadMessage = ref({
  tagline: 'File Uploading',
  message: ''
})
...
const handleFileUpload = (files)=>{
  ...
  if (files.length !== 0) {
    ...
    claimStore.uploadImage(preSignedUrl.value, files[0], (status)=>{
      if (status) {
        fileUploadMessage.value.tagline = 'Analysing damage and severity'
        fileUploadMessage.value.message = 'Please wait while we generate detailed results.'
...
const onDragOver = (e) => {
  e.preventDefault();
  e.stopPropagation();
  document.getElementById('file-upload-block').classList.add('bg-blue-200');
}
const onDragLeave = (e) => {
  e.preventDefault();
  e.stopPropagation();
  document.getElementById('file-upload-block').classList.remove('bg-blue-200');
}
const onDrop = (e) => {
  e.preventDefault();
  e.stopPropagation();
...
<template>
  <MainComponent>
    <div class="w-full">
      ...
      <div v-if="isLoader" role="status" class="flex flex-col items-center justify-center absolute -translate-x-1/2 -translate-y-1/2 top-2/4 left-1/2">
          ...
          <div v-if="isFileUploading" class="mt-3 font-medium text-2xl font-nunito app-text-secondary-400">{{ fileUploadMessage.tagline }}...</div>
          <div v-if="isFileUploading && fileUploadMessage.message" class="mt-2 font-normal font-nunito text-lg app-text-secondary-300">{{ fileUploadMessage.message }}</div>
      </div>
      <div v-else @click="onFilePicker($event)" @dragover="onDragOver($event)" @drop="onDrop($event)" @dragleave="onDragLeave($event)" id="file-upload-block" class="bg-white cursor-pointer hover:bg-blue-200 rounded-lg py-16 mt-10 upload-border flex justify-center items-center">
